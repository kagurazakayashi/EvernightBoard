...
/// 音量控制相關功能的 Mixin。
///
/// 此 Mixin 提供透過系統音量鍵來切換項目的控制邏輯：
/// - 音量增加時，切換到上一個項目
/// - 音量減少時，切換到下一個項目
///
/// 同時會關閉系統原生音量 UI，避免使用者在操作時看到系統音量提示，
/// 並在音量到達極限值時自動拉回到安全範圍內，讓音量鍵事件可以持續被觸發。
mixin HomeControllerVolume on ChangeNotifier {
  ...
  void _initVolumeControl() async {
    // 將目前的 mixin 宿主轉型為 HomeController，以便存取控制器內部狀態與方法
    ...
    FlutterVolumeController.addListener((volume) {
      ...
      // 若音量已到最大值，主動往回調降一點，避免卡在 1.0 導致後續無法再觸發增加事件
      if (volume >= 1.0) {
        FlutterVolumeController.setVolume(0.9);
        self._lastVolume = 0.9;
        // 若音量已到最小值，主動往上調高一點，避免卡在 0.0 導致後續無法再觸發減少事件
      } else if (volume <= 0.0) {
