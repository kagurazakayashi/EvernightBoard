...
### Create Start Menu Items and Desktop Shortcuts (Requires Windows or Linux)

You can use `.\shortcuts.ps1` (Windows) or `./shortcuts.sh` (Linux) to create startup/start menu items and desktop shortcuts.

1. Before running the script, ensure the following files are in the **same folder**:
   - **Linux**: `shortcuts.sh` (script), `icon.png` (icon file), `evernight_board` (executable).
   - **Windows**: `shortcuts.ps1` (script), `evernight_board.exe` (executable).
2. Grant execution permissions to the script:
   - **Linux**: Before running, grant execution permissions: `chmod +x shortcuts.sh` .
   - **Windows**: Before running, ensure PowerShell script execution is enabled: `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser` .

#### How to Use the Shortcut Script

Command format: `./script_name [action] [target] [mode]` .

1. Action
   - `add` (or `a`): Create shortcuts.
   - `remove` (or `r`): Remove shortcuts.
2. Target
   - `menu` (or `m`): System application menu (default).
   - `desktop` (or `d`): Desktop shortcut.
3. Mode
   - `--user` (or `-u`): Current user only (default).
   - `--system` (or `-s`): System-wide installation (requires Administrator/Root privileges).

#### Command Examples for the Shortcut Script

Linux `sh` examples:

- Add **menu item** for the **current user**: `./shortcuts.sh add` .
- Add **menu item** for the **entire system**: `sudo ./shortcuts.sh add menu --system` .
- Add both **menu item** and **desktop shortcut** for the **current user**: `./shortcuts.sh add menu desktop` .
- Add both **menu item** and **desktop shortcut** for the **entire system**: `sudo ./shortcuts.sh add menu desktop --system` .
- **Remove** menu item for the current user: `./shortcuts.sh remove` .
- **Remove** menu item and desktop shortcut for the entire system: `sudo ./shortcuts.sh remove menu --system` .

Windows `Windows PowerShell` examples:

- Change `./shortcuts.sh` in the examples above to `.\shortcuts.ps1` .
